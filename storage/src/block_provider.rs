...
use bytes::Bytes;
use chain::{
	BlockHeader, Transaction, Block,
	IndexedBlock, IndexedBlockHeader, 
	IndexedTransaction
};
...
pub trait BlockProvider: BlockHeaderProvider {
    ...
    /// resolve deserialized block body by block reference (number/hash)
    fn block(&self, block_ref: BlockRef) -> Option<Block>;

    /// returns true if store contains give block
