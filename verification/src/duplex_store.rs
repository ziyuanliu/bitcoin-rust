...
impl<'a> TransactionOutputProvider for DuplexTransactionOutputProvider<'a> {
    fn transaction_output(&self, prevout: &OutPoint, transaction_idex: usize) -> Option<TransactionOutput> {
        self.first.transaction_output(prevout, transaction_idex)
            .or_else(|| self.second.transaction_output(prevout, transaction_idex))
    }
