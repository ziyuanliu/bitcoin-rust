...
pub trait Synchronizer: Send {
    ...
    /// Returns true if permission for response is granted, but withoutmarking repsonse as sent.
...
impl ThresholdSynchronizer {
    ...
    fn within_threshold(&self, id: u32) -> bool {
        if self.to_grant_min <= self.to_grant_max {
            // if max is bigger than min, id must be in range [min, max)
...
impl ConfigurableSynchronizer {
    ...
    /// Although change happens immediately, responses with CONFIGURABLE_SYNCHRONIZER_THRESHOLD range
