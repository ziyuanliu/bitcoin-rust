...
enum State {
    /// Peer is sneding us messages && we wait for `PING_INTERVAL_S` to pass before sending ping request,
    WaitingTimeout(f64),
        ...
        match self.state {
            ...
            State::WaitingPong(time) => {
            	...
            	if now - time > MAX_PING_RESPONSE_TIME_S {
            	    trace!("closing connection to peer {}: no messages for last {} second", self.context.info().id, now - time);
            	    self.context.close();
            	}
            }
        }
