...
pub struct AddrProtocol {
    ...
    /// True if this is aconnection to the seednode && we should disconnect after receiving addr message
...
impl Protocol for AddrProtocol {
    ...
    fn on_message(&mut self, command: &Command, payload: &Bytes) -> Result<(), Error> {
        ...
        if command == &GetAddr::command() {
            ...
            match addr {
                Addr::V0(_) => {
                	unreachable!("This version of the protocol is not supported");
                },
                Addr::V31402(addr) => {
