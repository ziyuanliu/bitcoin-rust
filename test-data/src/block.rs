...
// local to this thread cannot be sent across
thread_local! {
...
impl<F> BlockBuilder<F> where F: Invoke<chain::Block> {
    ...
    pub fn into_raw(mut self, raw: &'static str) -> Self {
	...
	pub fn derived_transaction(self, tx_idx: usize, output_idx: u32) -> TransactionBuilder<Self> {
	    let tx = self.transactions.get(tx_idx).expect(&format!("using derived_transaction with the wrong index ({})", tx_idx)).clone();
	    TransactionBuilder::with_callback(self).input().hash(tx.hash()).index(output_idx).build()
	}

	// use vec![(0, 1), (0, 2), (1, 1)
	pub fn derived_transactions<I>(self, outputs: I) -> TransactionBuilder<Self> where I: IntoIterator<Item=(usize, u32)> {
	    let mut derives = Vec::new();
	    for (tx_idx, output_idx) in outputs {
	        derives.push(
	        	(
	        		self.transactions.get(tx_idx).expect(&format!("using derived_transaction with the wrong index ({})", tx_idx)).hash(),
	        		output_idx
	        	)
	        );
	    }
