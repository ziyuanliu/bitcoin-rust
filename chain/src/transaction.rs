...
/// Must be zero. it tells the serialization that the "list" following is empty
...
impl Transaction {
    ...
    pub fn is_final(&self) -> bool {
        ...
        // if locktime is 0, it is final or all of its inputs are final
        return self.lock_time == 0 || self.inputs.iter().all(TransactionInput::is_final)
    }
    ...
    pub fn has_witness(&self) -> bool {
        self.inputs.iter().all(TransactionInput::has_witness)
    }
