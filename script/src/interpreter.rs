...
fn verify_witness_program(
    ...
    if witness_version != 0 {
        if flags.verify_discourage_upgradeable_witness_program {
            return Err(Error::DiscourageUpgradeableWitnessProgram);
        }
    ...
    let witness_stack_len = witness.len();
	...
	while pc < script.len() {
	    ...
	    match opcode {
			...
			Opcode::OP_CHECKLOCKTIMEVERIFY => {
				...
				} else if flags.verify_discourage_upgradeable_nops {
				    return Err(Error::DiscourageUpgradeableNops);
				}
			},
			...
			Opcode::OP_NOP9 |
			Opcode::OP_NOP10 => {
				if flags.verify_discourage_upgradeable_nops {
				    return Err(Error::DiscourageUpgradeableNops);
				}
			},
...
mod tests {
	...
	fn witness_p2wpkh_with_future_version() {
		assert_eq!(Err(Error::DiscourageUpgradeableWitnessProgram),
			run_witness_test("".into(),
				...
				vec!["304402205ae57ae0534c05ca9981c8a6cdf353b505eaacb7375f96681a2d1a4ba6f02f84022056248e68643b7d8ce7c7d128c9f1f348bcab8be15d094ad5cadd24251a28df8001".into(),
					"0479be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798483ada7726a3c4655da4fbfc0e1108a8fd17b448a68554199c47d08ffb10d4b8".into()],
				VerificationFlags::default().verify_p2sh(true).verify_witness(true).verify_discourage_upgradeable_witness_program(true),
				0,
			));
	}
	...
	fn witness_unknown_program_version() {
		...
		let flags = VerificationFlags::default().verify_witness(true).verify_p2sh(true).verify_discourage_upgradeable_witness_program(true);
		assert_eq!(Err(Error::DiscourageUpgradeableWitnessProgram), run_witness_test_tx_test("51".into(), &tx, &flags, 1000, 0)
	...
	fn witness_33_bytes_witness_script_pubkey() {
		...
		let flags = VerificationFlags::default().verify_witness(true).verify_p2sh(true).verify_discourage_upgradeable_witness_program(true);
		assert_eq!(Err(Error::DiscourageUpgradeableWitnessProgram), run_witness_test_tx_test("6021ff25429251b5a84f452230a3c75fd886b7fc5a7865ce4a7bb7a9d7c5be6da3dbff".into(), &tx, &flags, 1000, 0));
	}
