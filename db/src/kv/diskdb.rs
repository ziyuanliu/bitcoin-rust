...
/// Compaction profile for db settings
...
impl CompactionProfile {
    /// Default profile suitable for ssd usage
    pub fn ssd() -> Self {
        CompactionProfile {
        	// 32 MB
...
pub struct DatabaseConfig {
    ...
    /// Cache size (in MiB) for specific columns,
...
impl Database {
    ...
    pub fn open<P>(config: DatabaseConfig, path: P) -> Result<Database, String> where P: AsRef<Path> {
    	...
    	// default cache size for the columns not specified
